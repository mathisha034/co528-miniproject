...
- [x] Issue 46: Localhost HTTP auth callback breakage (`cookie_not_found`) despite valid credentials — Implemented HTTPS dev runtime (`npm run dev:https` on `https://localhost:5174`), hardened Keycloak `react-web-app` redirect/origin config to HTTPS-only, added frontend insecure-origin auth guard, and fixed browser host mismatch by switching Keycloak URL to explicit `https://miniproject.local/auth` (`VITE_KEYCLOAK_URL`) instead of proxied relative `/auth`. Browser-driven flow now passes (`/`, `/profile`, `/notifications`, `/feed`, `/jobs`, `/events`), `e2e_admin` callback is stable (`3/3`), and `e2e_student`/`e2e_alumni` callbacks pass. ✅ **Tested & Verified 2026-03-18**
