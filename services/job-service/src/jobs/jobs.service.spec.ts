  ...
  describe('updateStatus — job transitions', () => {
    it('should allow open → closed', async () => {
      const mockJob = {
        ...
        save: jest.fn().mockResolvedValue({ status: JobStatus.CLOSED }),
      };
    ...
    it('should reject closed → open (invalid transition)', async () => {
      ...
      await expect(
        service.updateStatus(VALID_ID, { status: JobStatus.OPEN }),
  ...
  describe('updateApplicationStatus — app transitions', () => {
    it('should allow pending → reviewed', async () => {
      const mockApp = {
        ...
        save: jest
          .fn()
          .mockResolvedValue({ status: ApplicationStatus.REVIEWED }),
      };
      mockAppModel.findOne.mockResolvedValue(mockApp);
      await service.updateApplicationStatus(VALID_ID, VALID_ID, {
    ...
    it('should reject accepted → pending (invalid transition)', async () => {
      mockAppModel.findOne.mockResolvedValue({
        status: ApplicationStatus.ACCEPTED,
        save: jest.fn(),
      });
      await expect(
        service.updateApplicationStatus(VALID_ID, VALID_ID, {
          status: ApplicationStatus.PENDING,
        }),
      ).rejects.toThrow(BadRequestException);
    });
