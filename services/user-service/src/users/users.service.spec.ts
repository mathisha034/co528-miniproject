...
describe('UsersService', () => {
    ...
    describe('findMe', () => {
        it('should return user when found', async () => {
            mockUserModel.findOne.mockResolvedValue({ keycloakId: 'kc-123', email: 'a@b.com' });
    ...
    describe('updateMe', () => {
        ...
        it('should throw NotFoundException when user to update not found', async () => {
            ...
            await expect(service.updateMe('bad-id', {})).rejects.toThrow(NotFoundException);
        });
    });

    describe('findAll', () => {
        it('should return list of users', async () => {
            mockUserModel.find.mockReturnValue({ exec: jest.fn().mockResolvedValue([{ email: 'a@b.com' }]) });
