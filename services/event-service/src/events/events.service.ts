...
import { EventEntity, EventDocument, EventStatus } from './schemas/event.schema';
...
export class EventsService {
    ...
    async updateStatus(id: string, dto: UpdateEventStatusDto): Promise<EventDocument> {
    ...
    async rsvp(eventId: string, userId: string): Promise<EventDocument> {
        ...
        const updated = await this.eventModel
            .findByIdAndUpdate(eventId, { $addToSet: { rsvps: userObjId } }, { new: true })
            .exec();
