...
describe('EventsService', () => {
  ...
  describe('updateStatus — event transitions', () => {
    it('should allow upcoming → live', async () => {
      const mockEvent = {
        ...
        save: jest.fn().mockResolvedValue({ status: EventStatus.LIVE }),
      };
    ...
    it('should allow live → ended', async () => {
      const mockEvent = {
        status: EventStatus.LIVE,
        save: jest.fn().mockResolvedValue({ status: EventStatus.ENDED }),
      };
      mockEventModel.findById.mockResolvedValue(mockEvent);
    ...
    it('should reject ended → upcoming (invalid, backward)', async () => {
      ...
      await expect(
        service.updateStatus(VALID_ID, { status: EventStatus.UPCOMING }),
    ...
    it('should reject upcoming → ended (skip live)', async () => {
      ...
      await expect(
        service.updateStatus(VALID_ID, { status: EventStatus.ENDED }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('rsvp', () => {
    it('should throw BadRequestException for ended event', async () => {
      ...
      await expect(service.rsvp(VALID_ID, VALID_ID)).rejects.toThrow(
        BadRequestException,
      );
    });
